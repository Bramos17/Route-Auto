...
function ServiceAppointmentList() {
    ...
    const fetchUncanceledData = async () => {
        ...
        if (response.ok) {
            const data = await response.json();
            const filteredData = data.appointments.filter(object => (object["status"] !== "canceled" && object["status"] !== "finished"));
    ...
    const fetchUnfinishedData = async () => {
        ...
        if (response.ok) {
            const data = await response.json();
            const filteredData = data.appointments.filter(object => (object["status"] !== "finished" && object["status"] !== "canceled"));
            setAppointments(filteredData);
        }
    }
